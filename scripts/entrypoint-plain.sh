...
websocketd --binary --address 127.0.0.1 --port="${WSS_PORT}" /usr/local/bin/lima-websocket-bridge >/var/log/websocketd.log 2>&1 &
ws_pid="$!"

if [[ "${AUTO_START_LIMA}" =~ ^([Yy]|1)$ ]]; then
  case "${LIMA_TEMPLATE}" in
    *.yaml|*.qcow2|*.img|*.raw) lima_target="${LIMA_TEMPLATE}" ;;
